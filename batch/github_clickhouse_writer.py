...
class Node:
    ...
    type: str
    attrs: dict
    updated_at: str = field(default_factory=lambda: datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S"))
...
class Edge:
    ...
    type: str
    weight: float = 1.0
    recency: str = field(default_factory=lambda: datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S"))
...
def _normalize_event(event: GitHubEvent) -> Tuple[List[Node], List[Edge]]:
    ...
    if repo_name and owner:
        ...
        nodes.append(Node(id=full_repo_name, type="Repo", attrs={"name": repo_name, "owner": owner}))
        ...
        if "author" in data and isinstance(data["author"], dict):
             ...
             actor_login = data["author"].get("login") or data["author"].get("user", {}).get("login")
        elif "user" in data:
             # PR/Issue structure
        ...
        if actor_login:
            nodes.append(Node(id=actor_login, type="User", attrs={"login": actor_login}))
...
def write_events(events: Iterable[GitHubEvent]) -> int:
    ...
    for event in events:
        ...
        try:
            ...
            if nodes:
                ...
                if users: actor_id = users[0].id
                if repos: target_id = repos[0].id

            # Collect Nodes
            for node in nodes:
                attrs_json = json.dumps(node.attrs).replace("'", "'\'")
                node_rows.append(f"('{node.id}', '{node.type}', '{attrs_json}', '{node.updated_at}')")
                
            # Collect Edges
            for edge in edges:
                attrs_json = json.dumps(edge.attrs).replace("'", "'\'")
                edge_rows.append(f"('{edge.src}', '{edge.dst}', '{edge.type}', {edge.weight}, '{edge.recency}', '{attrs_json}')")
    ...
    if event_rows:
        sql = "INSERT INTO spectre.events (id, timestamp, actor_id, action_type, target_id, payload_json) VALUES " + ",".join(event_rows)
        client.query(sql)

    if node_rows:
        sql = "INSERT INTO spectre.nodes (id, type, attrs_json, updated_at) VALUES " + ",".join(node_rows)
        client.query(sql)

    if edge_rows:
        sql = "INSERT INTO spectre.edges (src, dst, type, weight, recency, attrs_json) VALUES " + ",".join(edge_rows)
        client.query(sql)
